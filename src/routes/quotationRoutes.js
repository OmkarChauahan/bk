...
router.get('/test', (req, res) => {
  res.json({ success: true, message: 'Quotation routes are working!' });
});
...
// PDF route — MUST be before /:id to avoid route conflict
...
router.post('/', authorize('Admin', 'Manager'), async (req, res) => {
  try {
    ...
    if (!customer || !items || !validUntil) {
      return res.status(400).json({ success: false, message: 'Please provide customer, items, and valid until date' });
    }

    let subtotal = 0, totalDiscount = 0, totalTax = 0;

    const processedItems = await Promise.all(items.map(async (item, index) => {
      if (!item.service || !String(item.service).match(/^[0-9a-fA-F]{24}$/)) {
        throw new Error(`Invalid or missing service ID at item ${index + 1}`);
      }

      const service = await Service.findById(item.service);
      if (!service) throw new Error(`Service not found: ${item.service}`);
    ...
    const invoiceNumber = `QT-${String(count + 1).padStart(4, '0')}`;
...
router.put('/:id', authorize('Admin', 'Manager'), async (req, res) => {
  try {
    ...
    if (['Accepted', 'Rejected'].includes(quotation.status)) {
      return res.status(400).json({ success: false, message: 'Cannot edit accepted or rejected quotations' });
    }
    ...
    if (items) {
      ...
      quotation.totalDue      = quotation.grandTotal;
    }
...
router.patch('/:id/status', authorize('Admin', 'Manager'), async (req, res) => {
  try {
    ...
    await quotation.save();
    res.json({ success: true, message: `Quotation ${status.toLowerCase()} successfully`, data: quotation });
  } catch (error) {
...
// ✅ Send quotation via email — actually sends email with PDF
